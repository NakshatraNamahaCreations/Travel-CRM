  ...
  const mut = useMutation({
    mutationFn: async () => {
      ...
      for (const f of files) {
        ...
        summaries.push(await importApi.upload(f, 'hotels', destIds));
      }
      return summaries;
    },
    onSuccess: (summaries) => {
  ...
  return (
    <div>
      <div className="flex items-center gap-2 border-b border-slate-200 bg-white px-6 py-3 text-sm">
        ...
        <span className="font-semibold text-slate-900">Upload Hotel Prices</span>
      ...
      <div className="px-6 py-6">
        <div className="max-w-3xl space-y-5">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-400">Imported hotels will be tagged with these destinations (city is auto-linked too).</p>
          </div>
          ...
          <div className="flex flex-wrap items-end gap-4">
            ...
            <div className="flex-1">
              <label className="label">Select csv file(s)</label>
              ...
              <button onClick={() => inputRef.current?.click()} className="flex w-full items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-slate-600 hover:border-brand-400">
                <UploadCloud size={16} className="text-slate-400" /> {files.length ? `${files.length} file(s) selected` : 'Choose Files — no file selected'}
              </button>
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => mut.mutate()} disabled={!files.length || mut.isPending} className="btn-primary">{mut.isPending ? 'Uploading…' : 'Upload Hotel Prices CSV'}</button>
            <button onClick={() => navigate('/services/hotel-prices')} className="text-sm font-medium text-slate-600 hover:text-slate-900">Cancel</button>
          </div>
